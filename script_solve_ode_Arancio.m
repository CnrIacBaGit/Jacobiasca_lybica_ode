...
% Stage-specific reproduction, mortality and development parameters.
beta_max   = 5;
muE_min    = 0.05;
alphaT_E   = 0.02;
alphaH_E   = 0.003;
...
%% 5. ENVIRONMENTAL RESPONSE PARAMETERS
% Logistic humidity/radiation modifiers and wind-response functions.
Tmin       = 11.0;
Tmax       = 35.0;
Tmin_spost = 22.0;
Hopt       = 60.0;
...
for c = 1:numel(cases)
    ...
    [E, N, A] = simulate_JL( ...
        ...
        muA_min, betaT_A, betaH_A, Topt_case, Hopt, Tmin_spost);
...
function [E,N,A] = simulate_JL( ...
    ...
    beta_max, muE_min, alphaT_E, alphaH_E, gammaE_max, ...
    muN_min, kW_mort, gammaN_max, alphaT_N, alphaH_N, ...
    muA_min, betaT_A, betaH_A, Topt, Hopt, Tmin_spost)
    ...
    for j = 1:nt-1
        T = Tvec(j); H = Hvec(j); R = Rvec(j); W = Wvec(j);
        ...
        muN = muN_min + alphaT_N*abs(T - Topt) + alphaH_N*abs(H - Hopt) + kW_mort*fW(W);
        muA = muA_min + betaT_A*abs(T - Topt) + betaH_A*abs(H - Hopt);

        E(j+1) = max(0, E(j) + ht*( beta*A(j)   - (muE + gammaE)*E(j) ));
        N(j+1) = max(0, N(j) + ht*( gammaE*E(j) - (muN + gammaN)*N(j) ));
        A(j+1) = max(0, A(j) + ht*( gammaN*N(j) - muA*A(j) ));
    end
end
